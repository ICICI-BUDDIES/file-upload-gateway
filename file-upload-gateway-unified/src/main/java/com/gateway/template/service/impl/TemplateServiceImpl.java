...
public class TemplateServiceImpl implements TemplateService {
    ...
    public Object getJsonByAppAndCategory(String appNameHash, String category) {
        ...
        if (t == null)
            throw new RuntimeException("Template not found for app and category");
        try {
            return mapper.readValue(t.getExtractedJson(), List.class);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private Object parseBytes(byte[] bytes, String ext) throws IOException {
        ...
        switch (ext.toLowerCase()) {
            ...
            case "txt":
            case "psv":
                System.out.println("📄 Using TXT parser (auto-detects delimiter)");
    ...
    private TemplateEntity createTemplate(MultipartFile file, RegistrationRequest request, String appNameHash)
            throws Exception {
        return createTemplate(file, request, appNameHash, null);
    }

    private TemplateEntity createTemplate(MultipartFile file, RegistrationRequest request, String appNameHash,
            String headerConfig) throws Exception {
    ...
    private void updateTemplate(TemplateEntity existingTemplate, MultipartFile file, RegistrationRequest request)
            throws Exception {
    ...
    public List<String> extractHeaders(MultipartFile file) {
        try {
            String ext = getExt(file.getOriginalFilename());
            System.out
                    .println("🔍 Extracting headers from file: " + file.getOriginalFilename() + ", extension: " + ext);

            if (!ext.equals("csv") && !ext.equals("xls") && !ext.equals("xlsx") && !ext.equals("txt") && !ext.equals("psv")) {
                throw new RuntimeException("Only CSV, XLS, XLSX, TXT, and PSV files are supported for header extraction");
            }
    ...
    private String validateTemplateField(String fieldName, String value, Map<String, Object> rules, int rowNumber) {
        ...
        System.out.println(String.format("Validating field '%s' with value '%s', type '%s', specialCharsAllowed: %s",
                fieldName, value, fieldType, specialCharsAllowed));
        ...
        switch (fieldType.toLowerCase()) {
            case "integer":
                ...
                if (!trimmedValue.matches("^-?\\d+$") && !trimmedValue.matches("^-?\\d+\\.0+$")) {
                    return String.format(
                            "Row %d: Field '%s' must be an integer (whole number) but contains '%s' in template",
                ...
                if (trimmedValue.contains(".")) {
                    try {
                        ...
                        if (doubleValue != Math.floor(doubleValue)) {
                            return String.format(
                                    "Row %d: Field '%s' must be a whole number but contains '%s' in template",
                                    rowNumber, fieldName, value);
                        }
                    } catch (NumberFormatException e) {
                        return String.format("Row %d: Field '%s' must be a valid integer but contains '%s' in template",
                                rowNumber, fieldName, value);
                    }
                }
                break;
            case "decimal":
                if (!trimmedValue.matches("^-?\\d+(\\.\\d+)?$")) {
                    return String.format("Row %d: Field '%s' must be a decimal number but contains '%s' in template",
                            rowNumber, fieldName, value);
                }
                break;
            case "number": // Keep for backward compatibility
                if (!trimmedValue.matches("^-?\\d+(\\.\\d+)?$")) {
                    return String.format("Row %d: Field '%s' must be a number but contains '%s' in template", rowNumber,
                            fieldName, value);
                }
                break;
            case "email":
                if (!trimmedValue.matches("^[A-Za-z0-9+_.-]+@([A-Za-z0-9.-]+\\.[A-Za-z]{2,})$")) {
                    return String.format("Row %d: Field '%s' must be a valid email but contains '%s' in template",
                            rowNumber, fieldName, value);
                }
                break;
            case "date":
                if (!trimmedValue.matches("^\\d{4}-\\d{2}-\\d{2}$|^\\d{2}/\\d{2}/\\d{4}$|^\\d{2}-\\d{2}-\\d{4}$")) {
                    return String.format("Row %d: Field '%s' must be a valid date but contains '%s' in template",
                            rowNumber, fieldName, value);
                }
                break;
        }

        // Special characters validation (only when special characters are NOT allowed
        // and field is not numeric/email)
        if (!specialCharsAllowed && !fieldType.toLowerCase().matches("(number|integer|decimal|email)")
                && trimmedValue.matches(".*[!@#$%^&*()\\[\\]{}|<>?/:;'\"~`].*")) {
            return String.format("Row %d: Field '%s' contains special characters which are not allowed in template",
                    rowNumber, fieldName);
        }
